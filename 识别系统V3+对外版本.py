...
class CowReIDSystem:
    ...
    def _get_or_generate_measurements(self, tid, real_cow_id):
        ...
        if real_cow_id in COW_BODY_MEASUREMENTS:
            ...
            # 生成带噪声的完整数据
            # base_measurements格式: [体重, 身高, 胸围, 腹围, 体斜长, 体直长, 十字部高]
            noisy_full = generate_measurements_with_noise(base_measurements)

            # 显示用数据：只取4个（去掉体重和腹围）
            # 显示: 身高、胸围、体斜长、体直长
            display_measurements = [
                noisy_full[0] if len(noisy_full) > 1 else None,  # 身高
                noisy_full[1] if len(noisy_full) > 2 else None,  # 胸围
                noisy_full[2] if len(noisy_full) > 4 else None,  # 体斜长
                noisy_full[4] if len(noisy_full) > 4 else None,  # 体直长
            ]

            # 完整数据字典（用于API）
            full_data_dict = {
                "Weight": noisy_full[0] if len(noisy_full) > 0 else 0,
                "BodyHeight": noisy_full[1] if len(noisy_full) > 1 else 0,
                "ChestAround": noisy_full[2] if len(noisy_full) > 2 else 0,
                "BellyAround": noisy_full[3] if len(noisy_full) > 3 else 0,
                "BodyDiagonal": noisy_full[4] if len(noisy_full) > 4 else 0,
                "BodyLength": noisy_full[5] if len(noisy_full) > 5 else 0,
                "CrossHeight": noisy_full[6] if len(noisy_full) > 6 else 0,
            }
